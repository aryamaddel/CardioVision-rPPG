...
export default function RecordScreen() {
  ...
  const fallbackVideoPromiseRef = useRef<Promise<{
    uri: string;
  } | null> | null>(null);
    ...
    return (
      <View style={[styles.permView, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.permBody, { color: colors.textSecondary }]}>
          CardioVision needs camera access to measure your heart rate from
          facial video.
        </Text>
        <TouchableOpacity
      ...
      <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
        ...
        {isRecording && (
          <View style={styles.bpmOverlay}>
            <Text style={styles.bpmNum}>
              {liveBpm !== null ? Math.round(liveBpm) : "--"}
            </Text>
            <Text style={styles.bpmLabel}>BPM</Text>
            <Text style={styles.instructText}>
              Stay still. Method: {liveMethod.toUpperCase()}
            </Text>
            {isTimerPaused && (
              <Text
                style={[
                  styles.pauseStatus,
                  pauseReason === "unknown_person"
                    ? styles.pauseStatusWarn
                    : null,
                ]}
              >
                {pauseReason === "unknown_person"
                  ? "Paused: unknown person detected"
                  : "Paused: primary face not detected"}
              </Text>
            )}
          </View>
        )}
        ...
        <View style={styles.bottomBar}>
          {isRecording ? (
            <View style={styles.ringWrap}>
              <Svg width={130} height={130} viewBox="0 0 130 130">
                ...
                <Circle
                  ...
                  cy={65}
                  r={CIRC_R}
                  stroke={timeLeft <= 5 ? "#FF6B35" : accent.primary}
                  strokeWidth={3}
                  fill="none"
            ...
            >
              <View
                style={[styles.recordBtnOuter, { borderColor: accent.primary }]}
              >
                <View
                  style={[
