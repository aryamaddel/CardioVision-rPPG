...
const STEPS = [
  {
    id: "upload",
    label: "Streaming capture",
    sub: "Live frames sent to backend websocket",
    detail:
      "The mobile app streams JPEG camera frames to the backend over websocket. If websocket is unavailable, the app falls back to HTTP upload mode.",
  },
  {
    id: "roi",
    label: "Face ROI extraction",
    sub: "MediaPipe landmark detection",
    detail:
      "MediaPipe Face Mesh detects 468 facial landmarks. Skin-colored regions (forehead, cheeks, nose) are segmented as Regions of Interest for signal extraction.",
  },
  {
    id: "pos",
    label: "POS algorithm",
    sub: "Plane-Orthogonal-to-Skin filtering",
    detail:
      "The POS (Plane Orthogonal to Skin) algorithm by Wang et al. (2017) separates the pulse signal from RGB channel variations using chrominance-based filtering.",
  },
  {
    id: "neural",
    label: "PhysFormer inference",
    sub: "Deep rPPG neural network",
    detail:
      "PhysFormer, a vision transformer pretrained on UBFC-rPPG, processes spatiotemporal facial features to extract a blood volume pulse (BVP) signal with higher SNR.",
  },
  {
    id: "fusion",
    label: "Signal fusion",
    sub: "POS + neural ensemble",
    detail:
      "POS and PhysFormer signals are weighted by their respective SNR values and fused into a single high-quality pulse waveform using adaptive weighting.",
  },
  {
    id: "hrv",
    label: "HRV analysis",
    sub: "RMSSD · SDNN · LF/HF extraction",
    detail:
      "Inter-beat intervals are computed from peak detection. Time-domain (RMSSD, SDNN) and frequency-domain (LF/HF ratio) HRV features are extracted.",
  },
  {
    id: "results",
    label: "Compiling results",
    sub: "Building your biometric report",
    detail:
      "All extracted metrics (BPM, HRV, stress level, confidence) are compiled into a structured JSON report for display on the results dashboard.",
  },
];
      ...
      <Animated.View
        style={[
          styles.stepRow,
          {
            ...
            opacity: state === "pending" ? 0.3 : fadeIn,
          },
        ]}
      >
        <View
        ...
        {state === "done" && (
          <View style={[styles.doneBadge, { backgroundColor: accent.ghost }]}>
            <Text style={[styles.doneBadgeText, { color: accent.primary }]}>
              DONE
            </Text>
          </View>
        )}
  ...
  useEffect(() => {
    const runPipeline = async () => {
      ...
      if (session.result) {
        ...
        if (pending) {
          pending.then((r) => resolveResult(r)).catch(() => resolveFallback());
        } else {
          resolveFallback();
        }
      }
      ...
      for (let i = 0; i < STEPS.length; i++) {
        ...
        Animated.timing(progressAnim, {
          toValue: pctTarget / 100,
          duration: durations[i] * 0.8,
          useNativeDriver: false,
        }).start();
        await new Promise((res) => setTimeout(res, durations[i]));
        if (i === STEPS.length - 1) {
          let waited = 0;
          while (!apiDone && waited < 6000) {
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.header}>
          ...
          <Text style={[styles.subtitle, { color: colors.textTertiary }]}>
            Processing your cardiac signal
          </Text>
          <PulseBar />
        </View>
        <View
        ...
        <Text style={[styles.expandHint, { color: colors.textMuted }]}>
          Tap completed steps for details
        </Text>
