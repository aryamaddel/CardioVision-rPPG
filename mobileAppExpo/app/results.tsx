...
function WaveformChart({
  ...
  const peakMarkers = peaks
    ...
    .map((p) => ({
      x: (p / (data.length - 1)) * CW,
      y: CH - ((data[p] - min) / range) * (CH - 20) - 10,
    }));
  return (
    <View style={styles.chartBox}>
      <Svg width={CW} height={CH}>
        ...
        <Path
          d={"M" + pts.join(" L") + ` L${CW},${CH} L0,${CH} Z`}
          fill={accent.ghost}
        />
        <Path
          d={"M" + pts.join(" L")}
...
function VitalCard({
  ...
  return (
    ...
    >
      <Text style={[styles.vitalLabel, { color: colors.textTertiary }]}>
        {label}
      </Text>
      <View style={{ flexDirection: "row", alignItems: "baseline", gap: 3 }}>
        ...
        {unit ? (
          <Text style={[styles.vitalUnit, { color: colors.textTertiary }]}>
            {unit}
          </Text>
        ) : null}
      </View>
      {sub && (
        <Text style={[styles.vitalSub, { color: colors.textMuted }]}>
          {sub}
        </Text>
      )}
    </Animated.View>
    ...
    >
      <View style={styles.tipHeader}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.tipSubtitle, { color: colors.textTertiary }]}>
            {tip.subtitle}
          </Text>
        </View>
        <Text style={[styles.tipDuration, { color: colors.textMuted }]}>
          {tip.duration}
        </Text>
      </View>
      {expanded && (
...
export default function ResultsScreen() {
  ...
  const initialResult = (() => {
    try {
      if (
        typeof params.resultJson === "string" &&
        params.resultJson.length > 0
      ) {
  ...
  const [resolvedResult, setResolvedResult] = useState<RPPGResult | null>(
    initialResult ?? null,
  );
  const [isLoadingResult, setIsLoadingResult] =
    useState<boolean>(!initialResult);
  ...
  const failReasonText =
    failReason === "identity_mismatch_detected"
      ...
      : (failReason ??
        "Insufficient valid face frames. Keep face centered and well-lit.");
  const stressScore =
  ...
  const meanIBI = ibi.length
    ? ibi.reduce((a: number, b: number) => a + b, 0) / ibi.length
    : null;
  ...
  const methodLabel = isLoadingResult
    ? "ANALYZING"
    : (result.method_used ?? "pos").replace("_", "+").toUpperCase();
  const fmt = (v: number | null, d: number = 1) =>
        ...
        >
          ...
          {isFailed && (
            <View
              style={[
                styles.failBanner,
                { backgroundColor: "#7F1D1D", borderColor: "#FCA5A5" },
              ]}
            >
          ...
          <View style={styles.heroBPM}>
            <Text style={[styles.heroBPMValue, { color: colors.textPrimary }]}>
              {bpm !== null ? Math.round(bpm) : "--"}
            </Text>
            <Text style={[styles.heroBPMUnit, { color: colors.textTertiary }]}>
              BPM
            </Text>
          </View>
          ...
          >
            <View style={styles.sectionHeader}>
              ...
              <Text style={[styles.hrvVal, { color: colors.textPrimary }]}>
                {fmt(rmssd, 0)}{" "}
                <Text style={[styles.hrvU, { color: colors.textTertiary }]}>
                  ms
                </Text>
              </Text>
            </View>
          ...
          <View style={styles.metricsRow}>
            ...
            >
              <Text
                style={[styles.metricBoxLabel, { color: colors.textTertiary }]}
              >
                Stress Score
              </Text>
              <Text
              ...
              >
                ...
                <Text
                  style={[styles.metricBoxUnit, { color: colors.textTertiary }]}
                >
                  /100
                </Text>
              </Text>
            ...
            >
              ...
              <Text
                style={[styles.metricBoxValue, { color: colors.textPrimary }]}
              >
                {fmt(sdnn, 0)}{" "}
                <Text
                  style={[styles.metricBoxUnit, { color: colors.textTertiary }]}
          ...
          >
            <View style={styles.sectionHeader}>
              ...
              <View style={[styles.badge, { backgroundColor: accent.ghost }]}>
          ...
          >
            ...
            <View style={styles.metricGrid}>
              ...
              <VitalCard
                ...
                value={fmt(lfhf, 2)}
                unit=""
                sub="Autonomic balance"
                delay={300}
                colors={colors}
              />
              <VitalCard
                label="Mean IBI"
                value={meanIBI !== null ? meanIBI.toFixed(0) : "--"}
                unit="ms"
                sub="Avg. inter-beat"
                delay={400}
                colors={colors}
          ...
          <TouchableOpacity
            style={[
              ...
              {
                backgroundColor: accent.primary,
                opacity: !resolvedResult || isDownloading ? 0.55 : 1,
              },
            ]}
            activeOpacity={0.85}
          ...
          >
            <Ionicons
              ...
              style={{ marginRight: 8 }}
            />
            <Text style={[styles.rescanText, { color: accent.primary }]}>
              Scan Again
            </Text>
          </TouchableOpacity>
          ...
          <Text style={[styles.disclaimer, { color: colors.textMuted }]}>
            ...
            a physician for clinical decisions.
          </Text>
        </ScrollView>
      </SafeAreaView>
    </View>
